 ...
 */
export default function ProjectCard({ tag, name, lead, body, links }: Project) {
  return (
      ...
      <div className="flex flex-wrap gap-x-8 gap-y-3 mt-6">
        {links.map(({ href, label }) => (
          <a
            ...
            target="_blank"
            rel="noopener"
            className="inline-flex items-center gap-2 text-sm text-foreground border-b border-accent pb-[2px] transition-[gap] duration-200 hover:gap-[14px] hover:text-accent print:hidden"
          >
            {label}
          </a>
        ))}
